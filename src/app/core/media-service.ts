...
export class MediaService {
  ...
  private _movies = signal<MediaItem[]>([]);

  async uploadDataToFirebase() {
    console.log('Starting upload... Please wait.');

    // We will call your collection 'media'
    const mediaCollection = collection(this.firestore, 'media');

    for (const item of this.MOVIE_DATA) {
      try {
        await addDoc(mediaCollection, item);
    ...
    console.log('🎉 ALL DONE! Check your Firebase Console.');
  }
}
